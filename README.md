...
| `app/api.py` | FastAPI: `/api/provinces`, `/api/schools`, `/api/search`, `/api/stats`, `POST /api/ingest`, `POST /api/reset`; serves `web/` |
| `app/analytics.py` | Pure analytics: per-province counters -> merged overall + per-subject/per-city metrics |
...
- **Statistics** (`/stats`): overall totals (provinces, schools, students, pass
  rate, mean average) + per-subject pass rates, with inline buttons to drill into
  any province's full per-subject breakdown. Public. Backed by `GET /api/stats`.
